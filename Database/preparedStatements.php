...
<head>
    <meta charset="UTF-8">
    <title>Insert into MySQL from PHP</title>
</head>
<body>
<?php
  ...
  $create_query = <<<_CREATE_
      CREATE TABLE IF NOT EXISTS PropertySeekers (
...
<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_error_msg) && empty($email_error_msg) &&
      ...
      # Store the information in the database so the realtors have access to it
      $stmt = $conn->prepare(
        "INSERT INTO `PropertySeekers` VALUES (?, ?, ?, ?)"
      );
      $stmt->bind_param("ssis", $name, $email, $phone, $property);
      $stmt->execute();
